...
$senalesDir = "C:\Users\bruno\Documents\programacion\autoescuela_Virtual\server\public\senales"
$progressFile = "C:\Users\bruno\Documents\programacion\autoescuela_Virtual\server\scripts\download-progress.json"

# Hardcoded URL map (from commons-urls.json analysis)
$urls = @{
    ...
    'p1b' = 'https://upload.wikimedia.org/wikipedia/commons/5/57/Spain_traffic_sign_p1b.svg'
    ...
    'p16a' = 'https://upload.wikimedia.org/wikipedia/commons/b/bf/Spain_traffic_signal_P-16a_%282022%29.svg'
    'p16b' = 'https://upload.wikimedia.org/wikipedia/commons/4/44/Spain_traffic_signal_P-16b_%282022%29.svg'
    ...
    'p20a' = 'https://upload.wikimedia.org/wikipedia/commons/f/f0/Spain_traffic_signal_P-20a.svg'
    'p21b' = 'https://upload.wikimedia.org/wikipedia/commons/d/df/Spain_traffic_signal_p21-b%2C_2023_set.svg'
    'p22b' = 'https://upload.wikimedia.org/wikipedia/commons/8/8e/Spain_traffic_signal_P-22b_%282023%29.svg'
    ...
    'r301' = 'https://upload.wikimedia.org/wikipedia/commons/4/48/Spain_traffic_signal_r301-50.svg'
    ...
    'r411' = 'https://upload.wikimedia.org/wikipedia/commons/7/76/Se%C3%B1al_r411_velocidad_m%C3%ADnima.svg'
    'r412' = 'https://upload.wikimedia.org/wikipedia/commons/7/75/Se%C3%B1al_r412_nieve.svg'
    'r413' = 'https://upload.wikimedia.org/wikipedia/commons/6/65/Se%C3%B1al_luces_obligatorias_r-413.svg'
...
foreach ($t in $targets) {
    $dest = Join-Path $senalesDir "$t.svg"
    if (Test-Path $dest -and (Get-Item $dest).Length -gt 100) { continue }
    if ($progress[$t] -eq 'ok') { continue }
    $missing += $t
}

Write-Output "Missing: $($missing.Count)"
$batch = $missing | Select-Object -Skip $Start -First $Count
Write-Output "Batch: $Start..($($Start + $batch.Count - 1))"

$ok = 0
foreach ($t in $batch) {
    $dest = Join-Path $senalesDir "$t.svg"
    if (Test-Path $dest -and (Get-Item $dest).Length -gt 100) {
        Write-Output "  $t.svg SKIP (exists)"
        $progress[$t] = 'ok'
    ...
    $orig = $urls[$t]
    if (-not $orig) { Write-Output "  $t.svg SKIP (no URL)"; continue }

    $prefix = "https://upload.wikimedia.org/wikipedia/commons/"
    $rel = $orig.Substring($prefix.Length)
    $name = [System.IO.Path]::GetFileNameWithoutExtension($rel)
    $thumb = "https://upload.wikimedia.org/wikipedia/commons/thumb/$rel/1280px-$name.svg.png"

    Write-Output "  $t.svg -> $thumb"

    Invoke-WebRequest -Uri $thumb -UserAgent "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36" -OutFile $dest

    if (Test-Path $dest -and (Get-Item $dest).Length -gt 100) {
        $sz = (Get-Item $dest).Length
        # Check if it's actually an image (PNG starts with 0x89)
        $bytes = [System.IO.File]::ReadAllBytes($dest)
        if ($bytes[0] -eq 0x89 -and $bytes[1] -eq 0x50 -and $bytes[2] -eq 0x4E -and $bytes[3] -eq 0x47) {
            $ok++
            $progress[$t] = 'ok'
            $progress | ConvertTo-Json | Set-Content $progressFile
            Write-Output "  OK (${sz} bytes)"
        } else {
            Write-Output "  CORRUPT (not PNG)"
            Remove-Item $dest -Force
            # Check if it's a 429 error
            $content = [System.IO.File]::ReadAllText($dest + '.tmp')
            if ($content -match '429|Too many') {
                Write-Output "  429 detected. Waiting 2min..."
                Start-Sleep -Seconds 120
                # Retry
                Write-Output "  Retrying $t..."
                Invoke-WebRequest -Uri $thumb -UserAgent "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36" -OutFile $dest
                if (Test-Path $dest -and (Get-Item $dest).Length -gt 100) {
                    $bytes2 = [System.IO.File]::ReadAllBytes($dest)
                    if ($bytes2[0] -eq 0x89) {
                        $ok++
                        $progress[$t] = 'ok'
                        $progress | ConvertTo-Json | Set-Content $progressFile
                        Write-Output "  OK after retry ($(Get-Item $dest).Length bytes)"
                    } else {
                        Write-Output "  FAIL after retry"
                    }
                }
            }
        }
